...
New nullable column on `application_errors` with a default of `NORMAL`:
...
Suggested changesets (all in a new `dropwizard-app-errors-migrations-v2.xml` or as
additional changesets in the existing file):

1. `0002-add-severity-column` — adds `severity` to `application_errors`
2. `0003-add-pinned-column` — adds `pinned` to `application_errors`
3. `0004-add-application-error-notes-table` — creates `application_error_notes` with FK

Separate MySQL variants will be needed as with `0001`.
